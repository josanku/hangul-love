...
## 2026-07-16 (6차 — 본문 옛한글 깨짐 수정)
- 진단: 서브셋 폰트는 한글 0자 누락(정상). 깨짐 원인 = ①본문에 U+1100 첫가끝(조합형) 자모 72,418자 → 웹폰트 로드 전(폴백)이나 옛한글 미지원 폰트에서 자모가 흩어져 깨짐, ②IPA 발음기호·이모지 107종(원본 폰트에도 없음)은 시스템 폴백.
- HarfBuzz 셰이핑 검증: 서브셋 폰트(woff2 압축해제 후)가 '한글'·'ᄋᆞ(아래아)'·'ᄒᆞᆫ' 모두 notdef 0으로 정상 합성 확인. (uharfbuzz는 woff2 미해제 → 오탐이었음.)
- 수정: ①본문 .txt NFC 정규화(현대 한글 조합형→완성형 14,462자, 어떤 폰트든 정상) ②본문 폰트 `<link rel=preload>`(첫 페인트 전 로드로 폴백 깨짐 방지) ③.serif/.jamo 폴백에 옛한글 지원 폰트(함초롬바탕/HCR Batang/Batang) 추가. 잔여 옛한글 57,956자는 웹폰트가 합성.
- 배포 확인: 프로덕션 HTML에 폰트 preload 포함, /read 본문 정상.
